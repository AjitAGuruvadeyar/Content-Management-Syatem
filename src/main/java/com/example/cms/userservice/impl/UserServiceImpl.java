...
return UserResponse.builder().userId(saveUser.getUserId()).username(saveUser.getUsername()).
		userEmail(saveUser.getEmail()).createdAt(saveUser.getCreatedAt()).lastModifiedAt(saveUser.getLastModifiedAt()).build();
		
	}


	public User mapToUser(UserRequest userRequest) {
		return User.builder().username(userRequest.getUserName()).email(userRequest.getUserEmail())
				.password(pe.encode(userRequest.getUserPassword())).build();
	}
}
